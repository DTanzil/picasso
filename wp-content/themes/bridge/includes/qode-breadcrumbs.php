		...
		if ($showOnHome == 1) echo '<div class="breadcrumbs"><div class="breadcrumbs_inner"><a'. $bread_style .' href="' . $homeLink . '">' . $home . '</a></div></div>';
			...
			if ( is_category() || qode_is_product_category()) {
			  ...
			  if ( get_post_type() != 'post' ) {
				...
				if(get_post_type() == 'product' && function_exists("is_woocommerce")) {
					// picasso edit: add Shop page link to product individual page
					global $woocommerce;
					$shop_id = get_option('woocommerce_shop_page_id');
					$shop= get_page($shop_id);
					echo '<a'. $bread_style .' href="' . $shop->guid . '">' . $shop->post_title . '</a>' . $delimiter;
				} 
