		...
		static function add_fields_diff_row( $fields, &$csv_values, $order_details, $item_id = null, $current_item = null ) {
			...
			foreach( $fields as $key =>$field ) {
				...
				switch ( $key ) {
					...
					// dania edit start
					// case 'wc_settings_tab_product_tax':
					// 	array_push( $csv_values, $current_item['line_tax']);
					// 	do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					// break;

					case 'wc_settings_tab_order_date': 
						array_push( $csv_values, $order_details->order_date);
						do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					break;
					...
					// not configured
					case 'wc_settings_tab_product_subtotal':
						array_push( $csv_values, $current_item['line_subtotal']);
						do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					break;

					case 'wc_settings_tab_product_total':
						array_push( $csv_values, $current_item['line_total']);
						do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					break;
					
					case 'wc_settings_tab_product_tax_subtotal':
						array_push( $csv_values, $current_item['line_subtotal_tax']);
						do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					break;

					case 'wc_settings_tab_product_tax_total':
						array_push( $csv_values, $current_item['line_tax']);
						do_action_ref_array( 'wsoe_after_value_'.$key, array( &$csv_values, $order_details, $item_id, $current_item ) );
					break;

					case 'wc_settings_tab_product_sku':
 						$prd = new WC_Product($current_item['product_id']);
						$sku = $prd->get_sku();
